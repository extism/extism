...
describe('test extism', () => {
  ...
  test('can free a plugin', async () => {
    await extism.withContext(async (ctx: extism.Context) => {
        ...
        let output = await plugin.call("count_vowels", "this is a test")
        plugin.free();
        await expect(() => plugin.call("count_vowels", "this is a test")).rejects.toMatch(/Plugin error/)
    })
  })
  ...
  test('withContext returns results', async () => {
    const count = await extism.withContext(async (ctx: extism.Context): Promise<number> => {
        const plugin = ctx.plugin(manifest())
        let output = await plugin.call("count_vowels", "this is a test")
        let result = JSON.parse(output.toString())
        return result['count']
  ...
  test('errors when function is not known', async () => {
    await extism.withContext(async (ctx: extism.Context) => {
        const plugin = ctx.plugin(manifest())
        await expect(() => plugin.call("i_dont_exist", "example-input")).rejects.toMatch(/Plugin error/)
    })
  })
