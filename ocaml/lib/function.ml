...
let v name inputs outputs ?user_data f =
  ...
  let free' =
    match user_data with None -> None | Some _ -> Some Root.release
  in
  let user_data =
    match user_data with
    | None -> null
    | Some user_data -> Root.create user_data
  in
  let f current inputs n_inputs outputs n_outputs user_data =
    let user_data =
      if is_null user_data then None else Some (Root.get user_data)
    in
